...
false positives. No migration and no logout: from v2.4.0 it is a plain image update.

- Two Web UI error answers no longer pass raw error text (which can contain host paths) to the
  browser; the details go to the log.
- The first-time setup page shows its messages as text instead of HTML.
- The image build workflow limits its GitHub token to read-only by default.

Coming from v2.3.1 or earlier? Read the v2.4.0 upgrade notes below first.
...
- Bumped **cryptography → 50.0.0** to resolve **CVE-2026-69247** (GHSA-g6cj-pr64-35w5): a Bleichenbacher timing/error oracle in PKCS#7 `EnvelopedData` decryption (High severity).
- No functional changes — dependency-only patch release across all platform images.
...
- Multiple source→target channel pairs with per-pair configuration
- Text, embeds, images, and videos are forwarded with translations
- Videos are re-uploaded as real Discord files for proper video player display
- YouTube, Twitter/X, Twitch, Reddit, Vimeo URLs get auto-previewed in translations
- Encrypted API key storage (independent of password, survives password changes)
- Rate limiting, retry logic, and auto-disable after consecutive failures
- Loop prevention (3 layers: bot message check, translated ID tracking, target channel exclusion)
...
### ✅ **v2.1.9 (2026-03-21) - Critical Channel Save Fix**
...
### ✅ **v2.1.8 (2026-03-21) - Channel Config & Startup Fixes**

🐛 **Bug Fixes:**
- **Admin Channel Persistence** (Issue #14): Fixed channels not being saved/loaded correctly after restart
  - Added fallback chain for channel loading: individual files → config.json → channels_config.json (legacy)
  - Auto-migration from legacy `channels_config.json` to individual channel files
  - Safe save order: new channel files are saved BEFORE old ones are deleted (prevents data loss)
  - Added safety check preventing deletion of all channels if saves fail
  - Reduced config.json writes from N+2 to 1 per save cycle (SMB/NFS stability)
- **Startup Port Diagnostics**: Removed false "Port 9374 NOT LISTENING" warning during startup (diagnostics ran before Waitress was ready)
- **Config Cache Staleness**: Bot now uses `force_reload=True` when loading config at startup to avoid stale cached data

📝 **Improvements:**
- Enhanced diagnostic logging for channel save/load operations
- Channel save return values are now checked and logged

---

### ✅ **v2.1.7 (2026-03-20) - Security Patch**

🔒 **Security Fixes:**
- **CVE-2026-32767** (Critical 9.8): Updated `expat` to 2.7.5-r0 — fixes XML parsing vulnerability
- **CVE-2026-32777, CVE-2026-32776** (Medium): Additional expat security patches
- **CVE-2026-32778** (Low): Expat minor security fix
- **CVE-2025-60876** (Medium): Busybox wget HTTP header injection mitigated (wget removed)
